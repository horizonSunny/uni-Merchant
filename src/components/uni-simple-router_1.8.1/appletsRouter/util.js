...
import { baiduApple, touTiao } from '../helpers/compile.js.js'
/**
 * 触发指定生命钩子
...
export const getFormatQuery = function(query = {}, getter = false) {
  if (Global.Router.CONFIG.encodeURI) {
    if (getter) {
      ...
      try {
        query = JSON.parse(
          decodeURIComponent(query.query || encodeURIComponent('{}'))
        )
      } catch (e) {
        query = JSON.parse(query.query)
      }
 ...
 */
export const pathOrNameToRoute = function(
  type,
  routes = Global.Router.CONFIG.routes
) {
...
export const ruleToUniNavInfo = function(rule, routes) {
  ...
  if (rule.constructor === String) {
    ...
    route = pathOrNameToRoute(
      rule['path'] || ((navType = 'name'), rule['name']),
      routes
    ) //两则必有其一 报错自己处理
    query = rule['query'] || rule['params'] || {}
  } else {
    return err(
      `传的什么乱七八糟的类型?路由跳转规则只认字符串 'path' , 对象 'path' , 对象 'name' `
    )
  }
  //路径处理完后   开始格式化参数
