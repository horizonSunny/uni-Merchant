import { appPlatform } from '../helpers/util.js.js'

import { Global } from '../helpers/config.js.js'
...
export const queryInfo = function(Vim) {
  ...
  if (Global.Router.CONFIG.encodeURI) {
    if (Vim.ONLAUNCH == null) {
      try {
        query = JSON.parse(
          decodeURIComponent(Vim.query.query || encodeURIComponent('{}'))
        )
      } catch (e) {
  ...
  switch (appPlatform()) {
    case 'H5':
      path =
        Vim.page &&
        ((ruleKey = 'aliasPath'), Vim.page.$page.path || '/' + Vim.page.route)
      break
    case 'BAIDU':
