import { vuelifeHooks } from './base.js.js'

import { afterHooks, beforeHooks, registerRouter } from './concat.js.js'

import { fromatRoutes } from './util.js.js'

import { err, warn } from '../helpers/warn.js.js'
...
const rewriteUniFun = function(Router) {
  ...
  uni.reLaunch = function({ url }) {
    if (url === '/') {
      warn(
        `H5端 uni.reLaunch('/')时 默认被重写了! 你可以使用 this.$Router.replaceAll() 或者 uni.reLaunch('/'?xxx)`
      )
      if (history.length > 1) {
