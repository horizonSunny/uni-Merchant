import { warn, err } from '../helpers/warn.js.js'
import { beforeEnterHooks } from './concat.js.js'
import {
  isObject,
  resolveRule,
  copyObject,
  ...
  strObjToJsonToStr,
  formatURLQuery
} from '../helpers/util.js.js'
import { queryInfo } from '../patch/applets-patch.js.js'
...
export const resolveRender = function(
  ...
  if (components != null) {
    warn(
      `vueRouterDev:false时 路由表配置中 ‘components’ 无效，\r\n\r\n ${JSON.stringify(
        item
      )}`
    )
  }
  ...
  if (component == null) {
    return err(
      `vueRouterDev:false时 路由表中 ‘component’ 选项不能为空：\r\n\r\n ${JSON.stringify(
        item
      )}`
    )
  }
  if (component.constructor === Function) {
    ...
    if (component.render == null || component.render.constructor !== Function) {
      err(
        `vueRouterDev:false时 路由表配置中 ‘render’ 函数缺失或类型不正确：\r\n\r\n ${JSON.stringify(
          item
        )}`
      )
    }
 ...
 */
export const fromatRoutes = function(
  routes,
  userRoute,
  { vueRouterDev, useUniConfig }
) {
...
export const formatUserRule = function(rule, selfRoutes, CONFIG) {
  let type = ''
  const ruleQuery =
    ((type = 'query'), rule.query || ((type = 'params'), rule.params)) ||
    ((type = ''), {})
  ...
  if (type != '') {
    ...
    let { query } = parseQuery(type, ruleQuery, false)
    if (CONFIG.encodeURI) {
...
export const getRouterNextInfo = function(to, from, Router) {
  ...
  if (H5.vueNext === false && H5.vueRouterDev === false) {
    ...
    let [toPath, fromPath] = [{}, {}]
    toPath[to.meta.PATHKEY] =
      to.meta.PATHKEY === 'path' ? `/${to.meta.pagePath}` : `${to.path}`
    fromPath[from.meta.PATHKEY] =
      from.meta.PATHKEY === 'path' ? `/${from.meta.pagePath}` : `${from.path}`
    ...
    const isEmptyTo =
      Object.keys(to.query).length != 0
        ? copyObject(to.query)
        : copyObject(to.params)
    const isEmptyFrom =
      Object.keys(from.query).length != 0
        ? copyObject(from.query)
        : copyObject(from.params)
    ...
    fromRoute = resolveRule(
      Router,
      fromPath,
      fromQuery,
      Object.keys(fromPath)[0]
    )
  } else {
...
export const encodeURLQuery = function(CONFIG, query, mode) {
  ...
  if (CONFIG.encodeURI === true && CONFIG.h5.vueRouterDev === false) {
    URLQuery = `?query=${encodeURIComponent(
      strObjToJsonToStr(query.query || '{}')
    )}`
  } else {
...
export const diffRouter = function(Router, vueRouter, useUniConfig, routes) {
  ...
  if (useUniConfig) {
    ...
    Routes.forEach((item, index) => {
      const path = item.path === '/' ? item.alias : item.path
      const vueRoute =
        Router.vueRoutes[path] ||
        Router.vueRoutes[item.path] ||
        Router.selfRoutes[path]
      ...
      CselfRoute.path =
        CselfRoute.aliasPath ||
        (item.path === '/' ? item.path : CselfRoute.path)
